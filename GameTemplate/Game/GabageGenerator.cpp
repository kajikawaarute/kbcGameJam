...
void GabageGenerator::Update()
{
	...
	if (m_timer >= 3.0f) {
		...
		botol->m_position.x = CMath::Lerp(botolS, -1500.0f, 1600.0f);
		//Z座標をランダムにする。
		//GetRandDoubleは0.0～1.をランダムに返してくる関数。
		float botolT = Random().GetRandDouble();
		//CMath::Leap関数は二つの値の間を線形補完する
		botol->m_position.z = CMath::Lerp(botolT, -2500.0f, 2500.0f);
		...
		paper->m_position.x = CMath::Lerp(PaperS, -1500.0f, 1500.0f);
		//Z座標をランダムにする。
		//GetRandDoubleは0.0～1.をランダムに返してくる関数。
		float PaperT = Random().GetRandDouble();
		//CMath::Leap関数は二つの値の間を線形補完する関数。
		paper->m_position.z = CMath::Lerp(PaperT, -2500.0f, 2400.0f);
		...
		paper2->m_position.x = CMath::Lerp(Paper2S, -3400.0f, -10.0f);
		//Z座標をランダムにする。
		//GetRandDoubleは0.0～1.をランダムに返してくる関数。
		float Paper2T = Random().GetRandDouble();
		//CMath::Leap関数は二つの値の間を線形補完する関数。
		paper2->m_position.z = CMath::Lerp(Paper2T, 3000.0f, 3500.0f);
